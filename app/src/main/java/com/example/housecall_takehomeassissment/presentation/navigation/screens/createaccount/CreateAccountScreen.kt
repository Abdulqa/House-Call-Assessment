...
fun CreateAccountScreen(
    navController: NavHostController, viewModel: CreateAccountViewmodel = hiltViewModel()
) {
    ...
    LaunchedEffect(Unit) {
        uiEvents.collectLatest {
            when (it) {
                is CreateAccountScreenUiCallbacks.AccountCreateFailed -> {
    ...
    Column(
        ...
        TextField(
            value = uiState.name,
            onValueChange = { viewModel.onEvent(CreateAccountEvent.OnNameChange(it)) },
            label = { Text("Name") },
        ...
        TextField(
            value = uiState.email,
            onValueChange = { viewModel.onEvent(CreateAccountEvent.OnEmailChange(it)) },
            label = { Text("Email") },
        ...
        TextField(
            ...
            onValueChange = { viewModel.onEvent(CreateAccountEvent.OnPasswordChange(it)) },
        ...
        Button(
            onClick = { viewModel.onEvent(CreateAccountEvent.OnCreateAccountClick) },
            modifier = Modifier
                .fillMaxWidth()
                .height(50.dp),
